...
int main(int argc, char **argv)
{
    ...
    complex double *YY = calloc(sizeof(complex double), n);
    while (1)
    {
        /* 標準入力からn個標本を読む */
        ...
        for (int i = 0; i < n; i++)
        {
            if (d * (i + 1) >= low2 && d * (i + 1) <= high2)
            {
                YY[i] = Y[(int)((1.0 * (d * (i + 1) - low2) * (high - low) / (high2 - low2) + low) / d)];
            }
            else
            {
                YY[i] = Y[i];
        ...
        double max = 5.0, min = 0.5;
        for (int i = 0; i < n; i++)
        {
            if (d * (i + 1) >= low2 && d * (i + 1) <= high2)
            {
                YY[i] *= min + (max - min) * (d * (i + 1) - low2) / (high2 - low2);
            }
        }
        print_complex(wp, YY, n);
        fprintf(wp, "----------------\n");
        /* IFFT -> Z */
        ifft(YY, X, n);
        /* 標本の配列に変換 */
